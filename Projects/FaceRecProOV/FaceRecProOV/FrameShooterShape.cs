...
{
    public partial class FrameShooterShape : Form
    {
        ...
        int camera = 0; //0 top camera, 1 bottom camera

        //signdetector
        static Image img2;
        ShapeDetector stopDetector;
        ...
        private void SwitchCameraButton_Click(object sender, EventArgs e)
        {
            if (camera == 0)
            {
                mc.sendCommand("SwitchCameraBottom");
            }
            else if (camera == 1)
            {
                mc.sendCommand("SwitchCameraTop");
            }
        }
